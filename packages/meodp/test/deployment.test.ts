...
import { waitForReport } from '../src/check/verify'
import { snapshot as report } from './notify-helpers'
...
test('deployment verification waits through missing, invalid and stale data before accepting the exact report', async () => {
  ...
  try {
    ...
    await assert.rejects(waitForReport(report, { url: 'file:///tmp/report.json' }), /HTTP\(S\)/)
    await assert.rejects(waitForReport(report, { url: 'https://user:password@example.com/report.json' }), /without credentials/)
    assert.equal(requests, 4)
    hasViewer = false
