import type { FeishuFetch } from '../src/notify/feishu'
import assert from 'node:assert/strict'
import { Buffer } from 'node:buffer'
import test from 'node:test'
import { createFeishuCard, createFeishuPayload, sendFeishuNotification, signFeishu } from '../src/notify/feishu'
...
test('invalid webhook destinations cannot receive messages', async () => {
  ...
  for (const invalid of [undefined, 'not-a-url', 'http://open.feishu.cn/open-apis/bot/v2/hook/test', 'https://example.com/open-apis/bot/v2/hook/test', `${webhook}?token=secret`, 'https://user:password@open.feishu.cn/open-apis/bot/v2/hook/test'])
    await assert.rejects(sendFeishuNotification(message, { webhook: invalid }, forbidden), /feishu.webhook/)
})
...
test('card uses plain text for site data, bounded summaries and accurate metadata', () => {
  const card = createFeishuCard({
    ...
    entries: Array.from({ length: 8 }, () => ({ kind: 'unavailable', reason: '新增不可访问', item: { ...report('unavailable').results[0], name: '<at id=all></at> **example**', url: 'https://example.com/', consecutiveFailures: 2 } })),
  }, { reportUrl: 'https://example.com/status/', timeZone: 'Asia/Shanghai' })
...
test('application configuration and authentication failures never attempt delivery', async () => {
  const forbidden = async () => assert.fail('Must not make a network request')
  await assert.rejects(sendFeishuNotification(message, { ...appOptions, transport: 'unexpected' }, forbidden), /feishu.transport/)
  await assert.rejects(sendFeishuNotification(message, { ...appOptions, appSecret: '' }, forbidden), /feishu.appSecret/)
