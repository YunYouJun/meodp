...
export function createFeishuCard(message: LinkNotification, { keyword = '', reportUrl, runUrl, reportLabel = '查看完整报告', timeZone = 'UTC', maxItems = 6 }: FeishuOptions = {}) {
  if (!Number.isSafeInteger(maxItems) || maxItems < 1 || maxItems > 6)
    throw new Error('maxItems must be between 1 and 6.')
  ...
  if (message.report) {
    ...
    elements.push({ tag: 'note', elements: [text(`${message.test ? '示例快照' : '检测完成'}：${time}（${timeZone}）\n检测环境：${truncate(message.report.observer, 80)}`)] })
    if (message.entries?.length) {
      elements.push({ tag: 'hr' }, { tag: 'markdown', content: '**状态摘要**' })
      for (const { item, reason } of message.entries.slice(0, maxItems)) {
      ...
      if (message.entries.length > maxItems)
        elements.push({ tag: 'note', elements: [text(`另有 ${message.entries.length - maxItems} 项，点击下方查看完整报告。`)] })
    }
    else {
  ...
  for (const [label, url] of [[reportLabel, reportUrl], ['查看本次运行', runUrl]] as const) {
    if (url)
...
export async function sendFeishuNotification(message: LinkNotification, options: FeishuOptions, fetcher: FeishuFetch = fetch) {
  ...
  if (!['app', 'webhook'].includes(transport))
    throw new Error('feishu.transport must be app or webhook.')
  if (transport === 'app') {
    for (const [name, value] of [['feishu.appId', options.appId], ['feishu.appSecret', options.appSecret], ['feishu.receiveId', options.receiveId]]) {
      if (!value?.trim())
        throw new Error(`Configure ${name} in the notification configuration.`)
    }
    ...
    if (!['open_id', 'user_id', 'union_id', 'email'].includes(receiveIdType))
      throw new Error('feishu.receiveIdType must be open_id, user_id, union_id, or email for direct messages.')
  ...
  try {
    ...
    throw new Error('Configure feishu.webhook in the notification configuration.')
  }
  if (webhook.protocol !== 'https:' || !['open.feishu.cn', 'open.larksuite.com'].includes(webhook.hostname)
    ...
    throw new Error('feishu.webhook must be an official Feishu/Lark custom bot webhook.')
  }
